    ...
    for index, swap in uniswap_df.iterrows():
        ...
        if uniswap_price > binance_price:
            ...
            if profit > PROFIT_THRESHOLD_USDT:
                print(f"发现机会! 利润: ${profit:.2f} USDT")
...
def save_results(results):
    if not results:
        print("未发现任何符合条件的套利机会。")
        return

    print(f"分析完成，共发现 {len(results)} 条套利机会。正在存入数据库...")
    results_df = pd.DataFrame(results)

    # 使用 to_sql 方法将整个DataFrame写入数据库表
    # 在每次运行时覆盖旧表 之后最好是append 当然 append 不能添加相同的！ 因为有唯一性约束在
    results_df.to_sql('arbitrage_opportunities', engine, if_exists='replace', index=False)

    print("结果已成功存入 arbitrage_opportunities 表。")
