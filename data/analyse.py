...
def load_data():
    ...
    uniswap_df['price'] = pd.to_numeric(uniswap_df['price'], errors='coerce').astype(float)
    uniswap_df['gas_price'] = pd.to_numeric(uniswap_df['gas_price'], errors='coerce').astype(float)
    ...
    binance_df['price'] = pd.to_numeric(binance_df['price'], errors='coerce').astype(float)
    ...
    logger.info(f"加载完成: {len(uniswap_df)} 条Uniswap记录, {len(binance_df)} 条币安记录")
    if not uniswap_df.empty:
        logger.info(f"Uniswap 数据时间范围: {uniswap_df.index.min()} -> {uniswap_df.index.max()}")
    if not binance_df.empty:
        logger.info(f"Binance 数据时间范围: {binance_df.index.min()} -> {binance_df.index.max()}")
...
def analyze_opportunities(uniswap_df, binance_df):
    ...
    for index, swap in uniswap_df.iterrows():
        ...
        if uniswap_price > binance_price:
            profit = calculate_profit_buy_cex_sell_dex(INITIAL_INVESTMENT_USDT, binance_price, uniswap_price, gas_price)
        ...
        # 条件: 币安价格 > Uniswap价格
            ...
            profit = calculate_profit_buy_dex_sell_cex(INITIAL_INVESTMENT_USDT, uniswap_price, binance_price, gas_price)
            if profit > PROFIT_THRESHOLD_USDT:
                logger.info(f"发现机会! 利润: ${profit:.2f} USDT")
                profitable_trades.append({
