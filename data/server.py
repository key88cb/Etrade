...
class TaskManager:
    ...
    def update_task_status(self, task_id: str, status: int):
        ...
        with self._lock:
            if task_id in self._tasks:
                ...
                if (
                    status == task_pb2.TASK_STATUS_RUNNING
                    and self._tasks[task_id]["start_time"] is None
                ):
                    self._tasks[task_id]["start_time"] = time.time()
                elif status in [
                    task_pb2.TASK_STATUS_SUCCESS,
                    task_pb2.TASK_STATUS_FAILED,
    ...
    def add_log(self, task_id: str, level: str, message: str):
        ...
        log_entry = task_pb2.TaskLog(timestamp=timestamp, level=level, message=message)
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        with self._lock:
            if task_id in self._tasks:
                ...
                if task["status"] in [
                    task_pb2.TASK_STATUS_QUEUED,
                    task_pb2.TASK_STATUS_RUNNING,
                ]:
                    task["cancel_flag"].set()
                    self.update_task_status(task_id, task_pb2.TASK_STATUS_CANCELED)
                    self.add_log(
                        task_id, "INFO", f"Task {task_id} cancellation requested"
                    )
                    return True
        return False
    ...
    try:
        ...
        try:
            ...
            task_manager.add_log(
                task_id, "INFO", f"Config loaded: {json.dumps(config)}"
            )
        except json.JSONDecodeError:
            task_manager.add_log(
                task_id, "WARN", f"Invalid JSON config, using empty config"
            )
            config = {}
        ...
        if task and task["cancel_flag"].is_set():
            task_manager.add_log(
                task_id, "INFO", f"Task {task_id} was canceled before execution"
            )
        ...
        task_manager.add_log(
            task_id, "INFO", f"Executing task logic for type: {task_type}"
        )
        ...
        if task_type == "DATA_IMPORT":
            ...
            if task and task["cancel_flag"].is_set():
                task_manager.add_log(
                    task_id, "WARN", f"Task {task_id} canceled during execution"
                )
                task_manager.update_task_status(task_id, task_pb2.TASK_STATUS_CANCELED)
                return
            time.sleep(0.5)

            task_manager.set_output_metrics(
                task_id, {"imported_rows": "1000", "files_processed": "5"}
            )
            task_manager.update_task_status(task_id, task_pb2.TASK_STATUS_SUCCESS)
            task_manager.add_log(
                task_id, "INFO", f"Task {task_id} completed successfully"
            )
            ...
            task_manager.set_output_metrics(
                task_id, {"report_file_path": "/tmp/report.pdf", "pages": "10"}
            )
            task_manager.update_task_status(task_id, task_pb2.TASK_STATUS_SUCCESS)
            task_manager.add_log(
                task_id, "INFO", f"Task {task_id} completed successfully"
            ...
            task_manager.set_output_metrics(
                task_id, {"result": "completed", "task_type": task_type}
            )
            task_manager.update_task_status(task_id, task_pb2.TASK_STATUS_SUCCESS)
            task_manager.add_log(
                task_id, "INFO", f"Task {task_id} completed successfully"
    ...
    def CancelTask(self, request, context):
        ...
        if task_manager.cancel_task(task_id):
            return task_pb2.CancelResponse(
                success=True, message=f"Task {task_id} cancellation requested"
            )
        else:
            task = task_manager.get_task(task_id)
            if task is None:
